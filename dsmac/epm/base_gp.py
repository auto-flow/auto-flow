...
import dsmac.epm.gp_base_prior
...
class BaseModel(AbstractEPM):
    ...
    ) -> List[List[dsmac.epm.gp_base_prior.Prior]]:
        ...
        while len(to_visit) > 0:
            ...
            if isinstance(current_param, sklearn.gaussian_process.kernels.KernelOperator):
                ...
                for i in range(hps[0].n_elements):
                    ...
                    if add_bound_priors:
                        if add_soft_bounds:
                            priors_for_hp.append(dsmac.epm.gp_base_prior.SoftTopHatPrior(
                               lower_bound=bounds[i][0], upper_bound=bounds[i][1], rng=self.rng,
                            ))
                        else:
                            priors_for_hp.append(dsmac.epm.gp_base_prior.TophatPrior(
                                lower_bound=bounds[i][0], upper_bound=bounds[i][1], rng=self.rng,
                            ))
