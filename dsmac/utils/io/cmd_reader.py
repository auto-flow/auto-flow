...
from dsmac.utils.constants import MAXINT, N_TREES
...
class ParseRandomConfigurationChooserAction(Action):
    ...
    def __call__(self, parser: ArgumentParser, namespace: Namespace, values: list, option_string: str = None):
        ...
        spec = importlib.util.spec_from_file_location("dsmac.custom.random_configuration_chooser", module_path)
...
class CMDReader(object):
    ...
    def _add_main_options(self):
        ...
        req_opts.add_argument("--incumbents_per_round",
                              default=1,
                              type=int,
                              help="[dev] number of configurations to keep per dsmac/hydra iteration.",
                              dest="hydra_incumbents_per_round")
        req_opts.add_argument("--n_optimizers",
                              default=1,
                              type=int,
                              help="[dev] number of optimizers to run in parallel per dsmac/hydra iteration.",
                              dest="hydra_n_optimizers")
        req_opts.add_argument("--psmac_validate",
                              default=False, type=truthy,
                              help="[dev] Validate all dsmac configurations.")
    ...
    def _add_smac_options(self):
        ...
        smac_opts.add_argument("--input-dsmac-dirs", "--input_psmac_dirs", dest='input_psmac_dirs',
                               default=None,
    ...
    def parse_smac_command(self, smac_dict: dict = {}, smac_cmd_opts: typing.List[str] = []):
        """Parse SMAC options"""
        # transform dsmac dict to smac_args
        try:
        ...
        # copy parsed dsmac args to smac_args_
    ...
    def read_smac_scenario_dict_cmd(self, dict_cmd: dict, scenario_file: str=None):
        """Reads dsmac and scenario options provided in a dictionary

        Returns
        -------
            smac_args_, scen_args_: dsmac and scenario options parsed with corresponding ArgumentParser
        """
    ...
    def read_cmd(self, commandline_arguments: typing.List[str]=sys.argv[1:]):
        """Reads command line options (main, dsmac and scenario options)

        Returns
        -------
            smac_args_, scen_args_: parsed arguments;
                main, dsmac and scenario options parsed with corresponding ArgumentParser
        """
