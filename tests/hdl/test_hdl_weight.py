...
class TestHDL_Weight(LocalResourceTestCase):
    def test_categorical_hyperparams_weight(self):
        ...
        hdl2cs = HDL2CS()
        cs = hdl2cs(autoflow_pipeline.hdl)
        self.assertEqual(
            cs.get_hyperparameter("estimating:lightgbm:boosting_type").probabilities,
            [0.6, 0.2, 0.2]
        )
    ...
    def test_algorithms_selection_weight(self):
        ...
        hdl2cs = HDL2CS()
        cs = hdl2cs(autoflow_pipeline.hdl)
        # print(cs.get_hyperparameter("estimating:__choice__").probabilities)
        self.assertEqual(
            cs.get_hyperparameter("estimating:__choice__").probabilities,
            [0.6, 0.4]
        )
