...
from autoflow.ambo.result import Result
from autoflow.ambo.utils import modify_timestamps
...
from autoflow.utils.config_space import config_regulation
...
class ResourceManager(StrSignatureMixin):
    ...
    def close_all(self):
        ...
        # self.close_all_logger.warning(CON
    ...
    def async_save(self, info, models_path, y_info, y_info_path, refit_model_path):
        model=info.pop("models")
        models_path = self.file_system.dump_pickle(model, models_path)
        y_info_path = self.file_system.dump_pickle(y_info, y_info_path)
        if refit_model_path:
            self.file_system.dump_pickle(info.pop("refit_model"), refit_model_path)
    ...
    def persistent_evaluated_model(self, info: Dict, model_id) -> Tuple[str, str, str]:
        ...
        if info.get("refit_model") is not None:
            refit_model_path = self.file_system.join(self.trial_dir,
                                                     f"{model_id}_final.{self.compress_suffix}")
        else:
            refit_model_path = ""
        # ----do dump---------------
        # async save
        t = Thread(target=self.async_save, args=(info, models_path, y_info, y_info_path, refit_model_path))
        t.start()
        # ----return----------------
        return models_path, refit_model_path, y_info_path
    ...
    def get_trial_model(self) -> pw.Model:
        class Trial(pw.Model):
            ...
            test_loss = pw.FloatField(default=ERR_LOSS)
            ...
            refit_model_path = pw.TextField(default="")
            ...
            class Meta:
                database = self.record_db
        if self.record_db is None:  # fixme: maybe close_all when dump pickle in thread?
            self.init_record_db()
    ...
    def get_best_loss(self):
        for time in range(3):
            try:
                self.init_trial_table()
                T = self.TrialModel
                records = list(T.select(pw.fn.Min(T.loss).alias("loss")).where(T.experiment_id == self.experiment_id).dicts())
                ret = records[0]["loss"]
                if ret is None:
                    return np.inf
                else:
    ...
    def insert_trial_record(self, info: Dict):
        ...
        models_path, refit_model_path, y_info_path = \
            self.persistent_evaluated_model(info, model_id)
        info.update(
            models_path=models_path,
            refit_model_path=refit_model_path,
    ...
    def do_insert_trial_record(self, user_id, task_id, hdl_id, experiment_id, budget_id, info: dict):
        ...
        trial_record = self.TrialModel.create(
            ...
            refit_model_path=info.get("refit_model_path", ""),
    ...
    def get_result_from_trial_table(
            ...
            user_id: int, budget_id: str, config_space
        ...
        records = Trial.select(
            ...
            # & (Trial.hdl_id == hdl_id)
            # & (Trial.budget_id == budget_id)
        ).order_by(Trial.experiment_id, Trial.config_id, Trial.budget).dicts()
        records = list(records)
        if len(records):
            self.logger.info(f"Fetch {len(records)} records from previous task, "
                             f"which will be used for warm start.")
        else:
        ...
        configs, _, activated_list = config_regulation(config_space, configs, return_activate=True)
        n_deactivates = np.count_nonzero(~np.array(activated_list))
        if n_deactivates > 0:
            self.logger.info(f"n_deactivates = {n_deactivates}")
        reg_records = []
        idx = 0
        for activated, record in zip(activated_list, records):
            if activated:
                record["config"] = configs[idx]
                reg_records.append(record)
                idx += 1
        # finish config_regulation
