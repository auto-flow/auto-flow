...
class ML_Workflow(Pipeline):
    ...
    def __init__(self, steps, should_store_intermediate_result=False, resource_manager=None):
        ...
        self.per_run_time_limit = float("inf")
        self.last_data = None
        self.use_workflow_cache = True
    ...
    def fit(self, X_train, y_train, X_valid=None, y_valid=None, X_test=None, y_test=None, fit_final_estimator=True,
        ...
        if self.fitted and self.last_data is not None:
            return self
        ...
        for (step_idx, step_name, transformer) in self._iter(
            ...
            cache_key = None
            start_time = time()
            if self.use_workflow_cache and getattr(transformer, "cache_intermediate", False):
            ...
            pipeline_cost_time = time() - pipeline_start_time
            if pipeline_cost_time > self.per_run_time_limit:
                self.time_limit_exceeded = True
                break

        if (fit_final_estimator and self.is_estimator) and not self.time_limit_exceeded:
    ...
    def procedure(
        ...
        try:
            return self._procedure(
                ml_task, X_train, y_train, X_valid, y_valid,
                X_test, y_test, max_iter, budget, should_finish_evaluation
            )
        except Exception as e:
            self.logger.error(str(e))
            self.logger.error(str(self.config))
            failed_info = get_trance_back_msg()
            status = "FAILED"  # todo: 实现 timeout(OK)， memory out
    ...
    def _procedure(
        ...
        # before K-CV , model.fit(fit_final_estimator=False) is called
        if self.fitted and max_iter > 0:
            # todo: transform 的过程中使用缓存（如，gbt_imputer）
            if self.last_data is None:
                self.logger.debug(f"fitted is True, self.last_data is None ")
        ...
        # problem-recording: autofeat div-0 err
        try:
            ...
            if ml_task.mainTask == "classification":
                pred_valid = self._final_estimator.predict_proba(X_valid) if X_valid is not None else None
                pred_test = self._final_estimator.predict_proba(X_test) if X_test is not None else None
            else:
                pred_valid = self._final_estimator.predict(X_valid) if X_valid is not None else None
            ...
            self.logger.error(e)
            raise Exception  # pred_test = -65535
            # pred_valid = -65535
        self.resource_manager = None  # 避免触发 resource_manager 的__reduce__导致连接池消失
        # when finish all training, clear last data
    ...
    def copy(self, share_items=("resource_manager")):
        tmp_list = []
        for item in share_items:
            tmp_list.append(getattr(self, item, None))
            setattr(self, item, None)
        res = deepcopy(self)
        for item, tmp in zip(share_items, tmp_list):
            setattr(res, item, tmp)
