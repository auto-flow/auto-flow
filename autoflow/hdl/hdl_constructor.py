...
class HDL_Constructor(StrSignatureMixin):
    ...
    def __init__(
            ...
            combine_rare=True,
            missing_rate_range=(0.2, 1, 0.2),
            included_cat_encoders=("encode.one_hot", "encode.ordinal"),
            num2normed_workflow=("scale.standard", "operate.keep_going"),
            normed2final_workflow=("operate.keep_going", "select.boruta", "generate.autofeat"),
            ...
            date2normed_workflow=None,  # todo: implement

    ):
        self.missing_rate_range = missing_rate_range
    ...
    def generic_recommend(self) -> Dict[str, List[Union[str, Dict[str, Any]]]]:
        ...
        if isinstance(self.num2normed_workflow, (tuple, list)):
            self.num2normed_workflow = {
                "num->normed": list(self.num2normed_workflow)
            }
        if isinstance(self.normed2final_workflow, (tuple, list)):
            self.normed2final_workflow = {
                "normed->final": list(self.normed2final_workflow)
            }
    ...
    def run(self, data_manager, model_registry=None, imbalance_threshold=2, specific_hdl_rule="default"):
        '''
        ...
        '''
        if specific_hdl_rule == "default":
            specific_hdl_rule = json5.loads((Path(__file__).parent / "specific_hdl_rule.json").read_text())
        elif isinstance(specific_hdl_rule, dict):
            self.logger.debug("Using dict-type specific_hdl_rule")
        elif specific_hdl_rule is None:
            self.logger.debug("specific_hdl_rule is None")
        else:
            raise ValueError("specific_hdl_rule can only be 'default', None or dict-type.")
        if specific_hdl_rule is not None:
            s, f = data_manager.X_train.shape
            final_hdl_rules = defaultdict(set)
            for rule, kwargs in specific_hdl_rule.items():
                if eval(rule):
                    self.logger.info(f"rule ({rule}) = True, apply hdl kwargs")
                    for k, v in kwargs.items():
                        if isinstance(v, str):
                            final_hdl_rules[k].add(v)
                        else:
                            final_hdl_rules[k] |= set(v)
            for k, v in final_hdl_rules.items():
        ...
        if imp_key in preprocessing_dict:
            missing_rate_range = list(self.missing_rate_range)
            preprocessing_dict[imp_key]["missing_rate"] = {
                "_type": "quniform",
                "_value": missing_rate_range,
                "_default": 0.4}  # fixme: 值得商榷的写法
        # 构造estimator
        estimator_dict = {}
