...
class BOHB(BaseConfigGenerator):
    def __init__(self, configspace: ConfigSpace, min_points_in_model=None,
        ...
        if self.min_points_in_model < self.n_variables + 1:
        ...
        for n_choices in self.transformer.n_choices_list:

            if n_choices > 0:
                self.kde_vartypes += 'u'
                self.vartypes += [n_choices]
            else:
                self.kde_vartypes += 'c'
                self.vartypes += [0]
    ...
    def _get_config(self, budget):
        ...
        if sample is None:
            try:
                ...
                # 用模型评估采样得到的num_samples个样本
                for i in range(self.num_samples):
                    # 从好样本中随机选择一个
                    idx = np.random.randint(0, len(kde_good.data))
                    # datum是一个M维向量，base-model是一个M维kde
                    ...
                    if not np.isfinite(val):
                        self.logger.debug('sampled vector has EI value %s' % (val))
                ...
                if len(vector_list) == 0:
                    ...
                    success = False
                    for best_ix in np.array(val_list).argsort():
                        best = val_list[best_ix]
                        best_vector = vector_list[best_ix]
                        # print("len:",len(vector_list))
                        try:
                            self.logger.debug(
                                'best_vector: {}, {}, {}, {}'.format(best_vector, best, l(best_vector), g(best_vector)))
                            # 处理离散变量
                            for i, hp_value in enumerate(best_vector):
                                if self.vartypes[i] > 0:
                                    best_vector[i] = int(np.rint(best_vector[i]))
                            converted_vector = self.transformer.inverse_transform(np.array(best_vector))
                            sample = ConfigSpace.Configuration(self.configspace, vector=converted_vector).get_dictionary()
                    ...
                    if not success:
                        self.logger.warning(f"Try to Convert vector to Configuration, "
                                            f"but failed {len(val_list)} times. Using Random sampling.")
                        sample = self.configspace.sample_configuration()
                        info_dict['model_based_pick'] = False
            except:
                self.logger.warning(
    ...
    def get_config(self, budget):
        ...
        while i < max_sample and self.configs.get(budget) is not None:
            ...
            config=ConfigSpace.Configuration(configuration_space=self.configspace,values=sample)
            array: np.ndarray = self.transformer.transform(config)
            X = np.array(self.configs[budget])
            array[np.isnan(array)] = -1
            X[np.isnan(X)] = -1
            if np.any(np.all(array == X, axis=1)):
                self.logger.info(
                    f"The sample already exists and needs to be resampled. It's the {i}-th time resampling. ")
    ...
    def new_result(self, job: Job, update_model: bool = True):
        """
        ...
        # todo: 消除常数项？
        self.configs[budget].append(self.transformer.transform(conf))
