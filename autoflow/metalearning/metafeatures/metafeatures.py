...
from autoflow.metalearning.util import get_data_preprocessor
...
class NumSymbols(HelperFunction):
    def _calculate(self, X, y, categorical):
        ...
        for i, column in enumerate(X.T):
            if categorical[i]:
                symbols_per_column.append(pd.Series(column).nunique())
    ...
    while len(to_visit) > 0:
        ...
        if name in npy_metafeatures:
            if X_transformed is None:
                ...
                DPP = get_data_preprocessor(np.arange(X.shape[1])[np.array(categorical)])
