...
class KDE4BO(BaseDensityEstimator):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        self.kde_vartypes = "".join(["u" if n_choices > 0 else "c" for n_choices in self.config_transformer.n_choices_list])
        ...
        if self.good_kde is None:
            good_kde_bw = np.zeros([len(self.config_transformer.n_choices_list)]) + 0.1
