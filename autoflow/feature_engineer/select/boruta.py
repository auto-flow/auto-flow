...
class BorutaFeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def _fit(self, X, y):
        ...
        if self.estimator is None:
            params = dict(
                ...
                warm_start=True  # for iterative training
            )
        ...
        while np.any(dec_reg == 0) and self.iter < self.max_iter:
            ...
            if self.n_estimators == 'auto':
                ...
                self.estimator.set_params(n_estimators=10)  # todo: 提炼step为一个参数
    ...
    def iterative_training(self, X, y):
        try:
            self.estimator = clone(self.estimator)
            step = 10
            self.estimator.set_params(n_estimators=step)
            for n_estimators in range(step, self.n_tree + step, step):
