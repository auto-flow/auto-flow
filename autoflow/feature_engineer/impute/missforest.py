...
class MissForest(BaseEstimator, TransformerMixin):
    ...
    * [3] https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.
    ...
    def transform(self, X):
        ...
        mask = _get_mask(X_, self.missing_values)
        if np.any(mask.sum(axis=0) >= (X_.shape[0])):
            logger.warning("One or more columns have all rows missing. Using SimpleImputer to do imputing.")
            from autoflow.feature_engineer.impute import SimpleImputer
            return SimpleImputer(consider_ordinal_as_cat=self.consider_ordinal_as_cat). \
                fit_transform(X)
