...
class AutoFeatureGenerator(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y, X_pool: Optional[List[pd.DataFrame]] = None):
        ...
        if pre_df.shape[1] > self.max_used_feats:
            ...
            base_model = base_model_cls(**base_model_params)
            for n_estimators in range(step, 50 + step, step):
                base_model.set_params(n_estimators=n_estimators)
                base_model.fit(X, y)
                if time() - self.start_time > time_left_for_fs:
                    break
            feature_importances = base_model.feature_importances_
            activated_indexes = np.argsort(-feature_importances)[:self.max_used_feats]
        else:
            activated_indexes = np.arange(pre_df.shape[1])
        df = pre_df.iloc[:, activated_indexes]
        self.pre_fs_cost_time=time()-self.start_time
        if X_pool:
            X_pool_new = []
            for X_ in X_pool:
                if X_ is None:
